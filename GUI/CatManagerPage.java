...
public class CatManagerPage extends JFrame implements ActionListener {
    ...
    JTextField type,petId,petOwnerId,petName,breed,gender,age,color,weight,searchField;
    ...
    class BG extends JPanel {
        Image img=new ImageIcon("./images/catdog_background.png").getImage();

        protected void paintComponent(Graphics g) {
            super.paintComponent(g);
            Graphics2D g2=(Graphics2D)g.create();
            g2.clipRect(0,60,440,getHeight()-60);
            int iw=img.getWidth(this),ih=img.getHeight(this);
            if(iw>0&&ih>0) {
                double s=Math.min(440.0/iw,(getHeight()-60.0)/ih);
                int w=(int)(iw*s),h=(int)(ih*s);
                g2.drawImage(img,(440-w)/2,200,w,h,this);
            }
            g2.dispose();
        }
    }
    ...
    void gui() {
        ...
        icon("pet.png",20,340);
        label("PET (CAT / DOG)",65,345,250,30)
                .setFont(new Font("Arial",Font.BOLD,18));
        ...
        label("Color",20,630,100,25); color=field(130,628,250);
        label("Weight (kg)",20,665,100,25); weight=field(130,663,250);
    ...
    Pet makePet(int id,int oid) {
        Object[] a={id,oid,petName.getText(),breed.getText(),gender.getText(),
                age.getText(),color.getText(),Double.parseDouble(weight.getText())};

        if(type.getText().equalsIgnoreCase("Cat"))
            return new Cat((int)a[0],(int)a[1],(String)a[2],(String)a[3],
                    (String)a[4],(String)a[5],(String)a[6],(double)a[7]);

        return new Dog((int)a[0],(int)a[1],(String)a[2],(String)a[3],
                (String)a[4],(String)a[5],(String)a[6],(double)a[7]);
    }
    ...
    void clearFields() {
        JTextField[] f={ownerId,ownerName,phone,email,address,type,petId,
                petOwnerId,petName,breed,gender,age,color,weight,searchField};
    ...
    public void actionPerformed(ActionEvent e) {
        try {
            if(e.getSource()==createOwner) {
                ...
                if(owners[id]==null) {
                    owners[id]=new Owner(id,ownerName.getText(),phone.getText(),
                ...
                if(pets[id]==null&&owners[oid]!=null&&
                        (t.equalsIgnoreCase("Cat")||t.equalsIgnoreCase("Dog"))) {
